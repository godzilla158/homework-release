...
| `setup --license` | Enter a license key. `hw`/`hwai`'s fetch step and `ai` require a valid key to run at all. `arrange`, `map`, `info`, `clean`, and `setup` itself are always unlimited |
